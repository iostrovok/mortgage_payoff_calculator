// Vercel serverless function for mortgage calculator
...
export default async function handler(req, res) {
	...
	try {
		...
		// Log user request (server-side only)
...
If the user asks about scenarios with different payment amounts or timeframes, provide specific calculations using the mortgage data provided.`;
		...
		// Log API request details (server-side only)
		...
		if (data.choices && data.choices.length > 0) {
			...
			// Log raw OpenAI response (server-side only)
			...
			// Log final enhanced response (server-side only)
