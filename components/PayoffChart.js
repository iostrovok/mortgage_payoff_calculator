...
const PayoffChart = ({ results }) => {
  ...
  const originalData = originalSchedule
    .filter((_, index) => index % sampleInterval === 0 || index === originalSchedule.length - 1)
  ...
  for (let i = 0; i < originalData.length; i++) {
    ...
    while (acceleratedIndex < acceleratedSchedule.length && 
           acceleratedSchedule[acceleratedIndex].paymentNumber <= originalPaymentIndex + 1) {
      acceleratedIndex++;
    }
