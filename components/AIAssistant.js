...
const AIAssistant = ({mortgageData}) => {
	...
	const handleSendMessage = async (messageText = inputText) => {
		...
		const userMessage = {
			id: Date.now(), type: 'user', text: messageText.trim(), timestamp: new Date(),
		};
		...
		try {
			...
			const assistantMessage = {
				id: Date.now() + 1, type: 'assistant', text: response, timestamp: new Date(),
			};
	...
	return (<KeyboardAvoidingView
		...
		>
			...
			>
				{messages.map((message) => (<View
						key={message.id}
						style={[
							styles.messageContainer, message.type === 'user' ? styles.userMessage : styles.assistantMessage]}
					>
						<Text style={[
							styles.messageText, message.type === 'user' ? styles.userMessageText : styles.assistantMessageText]}>
							{message.text}
						</Text>
						<Text style={[
							styles.timestamp, message.type === 'user' ? styles.userTimestamp : styles.assistantTimestamp]}>
							{formatTime(message.timestamp)}
						</Text>
					</View>))}

				{isLoading && (<View style={[styles.messageContainer, styles.assistantMessage]}>
						<Text style={styles.loadingText}>AI is thinking...</Text>
					</View>)}
			</ScrollView>

			{!mortgageData && (<View style={styles.noDataContainer}>
			...
			{mortgageData && (<>
					<View style={styles.quickQuestionsContainer}>
						...
						>
							{predefinedQuestions.map((question, index) => (<TouchableOpacity
									key={index}
...
const styles = StyleSheet.create({
	container: {
		...
		backgroundColor: '#007AFF', padding: 20, paddingTop: 40,
	}, headerTitle: {
		fontSize: 20, fontWeight: 'bold', color: '#fff',
	}, headerSubtitle: {
		fontSize: 14, color: '#B3D9FF', marginTop: 5,
	}, messagesContainer: {
		flex: 1, padding: 15,
	}, messagesContent: {
		paddingBottom: 20,
	}, messageContainer: {
		marginBottom: 15, maxWidth: '80%',
	}, userMessage: {
		alignSelf: 'flex-end', backgroundColor: '#007AFF', borderRadius: 18, borderBottomRightRadius: 5, padding: 12,
	}, assistantMessage: {
		...
		padding: 20, backgroundColor: '#fff', margin: 15, borderRadius: 12, alignItems: 'center',
	}, noDataText: {
		...
		fontSize: 14, color: '#007AFF',
	}, inputContainer: {
		flexDirection: 'row', padding: 15, backgroundColor: '#fff', alignItems: 'flex-end',
	}, textInput: {
		flex: 1,
		borderWidth: 1,
		borderColor: '#ddd',
