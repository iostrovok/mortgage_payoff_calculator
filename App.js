...
export default function App() {
	...
	const handleMortgageSubmit = (data) => {
		...
		const results = calculateMortgageDetails(data.principal, data.rate, data.term, data.additionalPayment, data.startDate);
	...
	return (<SafeAreaView style={styles.container}>
		<ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
			...
			{mortgageData && (<View style={styles.assistantButtonContainer}>
...
const styles = StyleSheet.create({
	container: {
		...
		height: 400, borderRadius: 12, overflow: 'hidden', shadowColor: '#000', shadowOffset: {
			width: 0, height: 2,
