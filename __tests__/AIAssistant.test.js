...
describe('AIAssistant', () => {
	...
	test('should send message when quick question button is pressed', async () => {
		...
		await waitFor(() => {
			expect(openaiService.getOpenAIResponse).toHaveBeenCalledWith('How much sooner if I add $200 per month?', mockMortgageData);
		});
	...
	test('should show loading state while waiting for response', async () => {
		...
		openaiService.getOpenAIResponse.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve('Response'), 100)));
