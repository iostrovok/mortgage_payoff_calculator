...
describe('OpenAI Service', () => {
  ...
  describe('getOpenAIResponse', () => {
    test('should successfully make API request and return response', async () => {
      const mockResponse = {
        response: 'Adding $300 per month would save you 5 years and $50,000 in interest.',
        tokens: 150,
        model: 'gpt-3.5-turbo'
      };
      ...
      expect(fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          body: expect.stringContaining(userQuery)
        })
      );
    ...
    test('should handle null mortgage data', async () => {
      const mockResponse = {
        response: 'Please provide mortgage details',
        tokens: 50,
        model: 'gpt-3.5-turbo'
      };
  ...
  describe('Fallback behavior', () => {
    test('should provide meaningful fallback for common queries', async () => {
      ...
      const testCases = [
        {
          query: 'How much sooner if I add $500 per month?',
          expectedContains: ['$500', 'sooner']
        },
        {
          query: 'What total interest will I save with $200?',
          expectedContains: ['$200', 'save']
        },
        {
          query: 'Can I finish my mortgage in 15 years?',
          expectedContains: ['15 years']
        }
      ];
