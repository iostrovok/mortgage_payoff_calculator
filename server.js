...
const enhanceWithCalculations = (aiResponse, userQuery, mortgageData) => {
  ...
  if (amount && (userQuery.toLowerCase().includes('add') || userQuery.toLowerCase().includes('extra'))) {
    try {
      ...
      aiResponse += `\n\n💰 Specific calculation: Adding ${formatCurrency(amount)} per month would save you ${timeString} and ${formatCurrency(result.interestSaved)} in total interest.`;
    } catch (error) {
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (!OPENAI_API_KEY) {
      console.log("OpenAI API key not configured. Please check your environment variables.");
      return res.status(500).json({ 
        error: "OpenAI API key not configured",
        message: "OpenAI is not available. Please try again later." 
      });
    }
